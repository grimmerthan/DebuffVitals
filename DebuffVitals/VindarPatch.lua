...
local function _convBack(obj)
	...
	elseif obj=="" then
		...
		ret = euroNormalize(obj)
		if ret == nil then
			ret = ""
			if obj:byte(1,1) == 35 then obj = string.sub(obj,2,-1) end
			while string.len(obj)~=0 do
				if 		obj:byte(1,1) ~= 35 then ret = ret .. string.char(obj:byte(1,1)); obj = string.sub(obj,2,-1)
				elseif obj:byte(2,2) == 35 then  ret = ret .. "#"; obj = string.sub(obj,3,-1)
				else
